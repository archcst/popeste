...
- 设置中复选快捷键方案：默认只启用 ↑ ↓ ← →；Emacs 用 Ctrl+N/P 选择、Ctrl+F/B 切换下一个／上一个标签；Vim 用 Ctrl+J/K 选择、Ctrl+L/H 切换下一个／上一个标签。三项可组合启用，勾选状态与按键提示同步更新。Enter 插入、Esc 取消；正文编辑与中文组词保留原有输入行为。
...
展开后，搜索框下方显示「全部」「最近使用」及自定义标签。左右键切换标签，上下键选择短语，搜索只筛选当前标签。自定义标签可拖动排序，顺序自动保存并与编辑页同步。

编辑页可多选胶囊标签，再次点击取消选择，「无标签」清空选择，点击「＋」新增标签。鼠标悬停时，标签右侧展开铅笔按钮，点击即可在同一面板修改名称、颜色或删除标签，回车保存、Esc 取消。重命名会同步更新关联短语；删除标签时保留关联短语及其余标签。标签显示在候选短语左侧。编辑面板可选择色块或输入十六进制色值自定义颜色，颜色随配置保存。在自定义标签下新建短语会自动带上该标签。「最近使用」按成功插入的时间倒序排列；旧数据不推测历史使用时间，从本版本开始记录。
...
| `prompts.json` | 正文、ID、置顶、标签、使用次数、最近使用时间、更新时间；版本 3 |
...
辅助功能用于读取光标附近的位置和模拟 Command+V。浮窗优先出现在光标上方；列表优先向下展开，下方空间不足时向上展开。无法取得光标位置时回退鼠标附近；缺少授权时可以继续管理和复制。开发构建更换签名后，若系统开关开启但应用仍未获授权，可移除旧的辅助功能条目后重新添加当前应用。
