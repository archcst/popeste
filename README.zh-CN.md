...
编辑页可多选胶囊标签，再次点击取消选择，「无标签」清空选择，点击「＋」新增标签。鼠标悬停标签后点击铅笔，即可原地编辑名称，回车保存、Esc 取消；重命名会同步更新关联短语。清空名称后按回车删除标签，关联短语及其余标签保留。标签显示在候选短语左侧。右键标签可选择色块或输入十六进制色值自定义颜色，颜色随配置保存。在自定义标签下新建短语会自动带上该标签。「最近使用」按成功插入的时间倒序排列；旧数据不推测历史使用时间，从本版本开始记录。
