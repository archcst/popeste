...
- 列表默认只显示搜索框，输入已上屏内容后展开；↓（启用 Emacs 方案时也可用 Ctrl+N）展开全部短语并选中第一条。清空搜索保持展开；设置中开启「呼出时展开」后，下次呼出直接显示标签和候选短语，关闭则只显示搜索框（默认）。⌘N 和 ⌘, 可直接进入新建和设置。
...
编辑页可多选胶囊标签，再次点击取消选择，「无标签」清空选择，点击「＋」新增标签。鼠标悬停 500 毫秒后，标签右侧展开铅笔按钮，点击打开标签编辑弹出层，可修改名称、颜色或通过底栏垃圾桶删除标签，回车保存、Esc 取消。重命名会同步更新关联短语；删除标签时保留关联短语及其余标签。标签显示在候选短语左侧。颜色栏左侧按钮打开 macOS 颜色选择器，右侧提供八种预设颜色，颜色随配置保存。在自定义标签下新建短语会自动带上该标签。「最近使用」按成功插入的时间倒序排列；旧数据不推测历史使用时间，从本版本开始记录。
