...
8 组独立 Swift 测试覆盖数据、导入导出、迁移、配置持久化、编辑草稿和屏幕位置。无需 XCTest 或完整 Xcode。另有 21 项 JavaScript 回归测试覆盖输入法事件、Vim 编辑及保存确认，运行测试脚本需安装 Node.js。
...
| 文件 | 内容 |
| --- | --- |
| `config.json` | 全局快捷键、浮窗大小、首次使用标记、登录启动偏好、外观、语言、快捷键方案、Vim 编辑模式 |
