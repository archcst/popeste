...
final class Settings: NSObject {
    let window = NSWindow(contentRect: NSRect(x: 0, y: 0, width: 1080, height: 720), styleMask: [.titled, .closable], backing: .buffered, defer: false)
    let hotKey: HotKey
    let configuration: Configuration
    let interface = WebInterface(mode: "settings")
    var back: (() -> Void)?
    var importAction: (() -> Void)?
    init(hotKey: HotKey, configuration: Configuration) {
        self.hotKey = hotKey; self.configuration = configuration; super.init()
        window.title = "Popaste · 设置"; interface.attach(to: window)
        interface.action = { [weak self] name, payload in self?.handle(name, payload) }
        update()
    }
    func show() { update(); NSApp.activate(ignoringOtherApps: true); window.makeKeyAndOrderFront(nil) }
    private func handle(_ name: String, _ payload: [String: Any]) {
        do {
            switch name {
            case "ready", "refresh": update()
            case "back": window.orderOut(nil); back?()
            case "import": importAction?()
            ...
            case "permission": _ = AXIsProcessTrustedWithOptions([kAXTrustedCheckOptionPrompt.takeUnretainedValue() as String: true] as CFDictionary)
            case "systemSettings": NSWorkspace.shared.open(URL(string: "x-apple.systempreferences:com.apple.preference.security?Privacy_Accessibility")!)
            default: break
            }
    ...
    func update() {
        interface.state = ["prompts": [], "shortcut": hotKey.shortcut.label, "size": configuration.value.pickerSize.rawValue, "trusted": AXIsProcessTrusted(), "login": SMAppService.mainApp.status == .enabled, "loginPending": SMAppService.mainApp.status == .requiresApproval, "appearance": configuration.value.appearance ?? "system", "dark": interfaceDark(configuration.value.appearance ?? "system")]
    }
}
